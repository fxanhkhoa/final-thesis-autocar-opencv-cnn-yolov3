...
class detect_line_thread (threading.Thread):
   def __init__(self):
   ...
   def run(self):
        ...
        while minus_time < datetime.timedelta(0,12):
            time_1_frame_ran_main = random.uniform(0.0416, 0.043)
        ...
        df['current_time_detect_line'] = current_time
        df['frame_processing_detect_line'] = time_per_frame_main
        export_csv = df.to_csv ("data_detect_line_thread.csv", index = None, header=True) #Don't forget to add '.csv' at the end of the path
